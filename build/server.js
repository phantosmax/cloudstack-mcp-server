...
class CloudStackMcpServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Error executing tool ${request.params.name}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async handleListVirtualMachines(args) {
    ...
    async handleGetVirtualMachine(args) {
...
Memory: ${vm.memory}MB
Created: ${vm.created}
${vm.nic ? `\nNetwork:\n${vm.nic.map((n) => `  IP: ${n.ipaddress}, Network: ${n.networkname}`).join('\n')}` : ''}`,
                },
            ],
        };
    }
    async handleStartVirtualMachine(args) {
    ...
    async handleStopVirtualMachine(args) {
    ...
    async handleRebootVirtualMachine(args) {
    ...
    async handleDestroyVirtualMachine(args) {
        if (!args.confirm) {
            throw new McpError(ErrorCode.InvalidRequest, 'DESTRUCTIVE ACTION BLOCKED: This will permanently delete the virtual machine. Please set confirm=true to proceed. THIS CANNOT BE UNDONE.');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to destroy VM: ${error.message}`);
        }
    }
    async handleListZones(args) {
    ...
    async handleDeployVirtualMachine(args) {
        try {
            ...
            if (args.zoneid && !args.networkids) {
                ...
                if (zone?.networktype === 'Advanced') {
                    ...
                    if (networkList.length > 0) {
                        ...
                        throw new McpError(ErrorCode.InvalidRequest, 'This is an Advanced zone but no networks are available. Please create a network first.');
                    }
                }
            }
            // Ensure we have required parameters
            if (!args.serviceofferingid || !args.templateid || !args.zoneid) {
                throw new McpError(ErrorCode.InvalidRequest, 'Missing required parameters: serviceofferingid, templateid, and zoneid are required');
            }
            // Deploy the VM
            ...
            if (error.message?.includes('530')) {
                throw new McpError(ErrorCode.InvalidRequest, 'VM limit reached for your account. Please destroy some existing VMs before creating new ones.');
            }
            else if (error.message?.includes('431')) {
                throw new McpError(ErrorCode.InvalidRequest, 'Missing required parameter. For Advanced zones, you may need to specify networkids.');
            }
            throw error;
        }
    }
    // VM Advanced Operations Handlers
    async handleScaleVirtualMachine(args) {
    ...
    async handleMigrateVirtualMachine(args) {
    ...
    async handleResetPasswordVirtualMachine(args) {
    ...
    async handleChangeServiceOfferingVirtualMachine(args) {
    ...
    async handleCreateSnapshot(args) {
            ...
            content: [
                {
                    ...
                    text: `Found ${snapshotList.length} snapshots:\n\n${snapshotList
                        .map((snapshot) => `• ${snapshot.name} (${snapshot.id})\n  Volume: ${snapshot.volumename}\n  Type: ${snapshot.snapshottype}\n  State: ${snapshot.state}\n  Created: ${snapshot.created}\n`)
                        .join('\n')}`,
                },
            ],
        };
    ...
    async handleCreateNetwork(args) {
    ...
    async handleListPublicIpAddresses(args) {
    ...
    async handleEnableStaticNat(args) {
    ...
    async handleCreateFirewallRule(args) {
    ...
    async handleListLoadBalancerRules(args) {
    ...
    // Monitoring Handlers
    async handleListVirtualMachineMetrics(args) {
            ...
            content: [
                {
                    ...
                    text: `System Capacity:\n\n${capacity
                        .map((cap) => `• ${cap.type}: ${cap.percentused}% used\n  Used: ${cap.capacityused} / ${cap.capacitytotal}\n  Zone: ${cap.zonename}\n`)
                        .join('\n')}`,
                },
            ],
        };
    }
    async handleListAsyncJobs(args) {
    ...
    async handleListUsageRecords(args) {
    ...
    async handleListStoragePools(args) {
    ...
    async handleListSystemVms(args) {
    ...
    async handleListServiceOfferings(args) {
    ...
    // Security Handlers
    async handleListSSHKeyPairs(args) {
    ...
    async handleCreateSSHKeyPair(args) {
    ...
    async handleListSecurityGroups(args) {
    ...
    async handleCreateSecurityGroupRule(args) {
